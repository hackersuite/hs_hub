...
export class ReservedHardwareService implements ReservedHardwareServiceInterface {
   ...
   */
	...
	private readonly getAllReservations = async (): Promise<ReservedHardwareItem[]> => {
		try {
		...
		await this.reservedHardwareRepository.manager.transaction(async transaction => {
			...
			if (response.raw.affectedRows != 1) {
				...
				const reservationForToken = await transaction
					...
					.where('reservation.reservationToken = :token', { token })
					.getOne();
				if (reservationForToken) { throw new ApiError(HttpResponseCode.INTERNAL_ERROR, 'Could not cancel reservation, please inform us that this error occured.'); }
			}
			await transaction
   ...
   */
	public isReservationValid = (expiryDate: Date): boolean => Date.now() <= expiryDate.getTime();
}
